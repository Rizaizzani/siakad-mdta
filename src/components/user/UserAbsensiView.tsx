...
export default function UserAbsensiView() {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-wrap gap-3 items-center">
        ...
        {(filterBulan || filterTanggal || filterKelas !== 'Semua') && (
          <button
            onClick={() => { setFilterKelas('Semua'); setFilterBulan(''); setFilterTanggal(''); }}
            className="text-xs text-slate-500 hover:text-rose-500 underline"
          >
            Reset
          </button>
        )}
