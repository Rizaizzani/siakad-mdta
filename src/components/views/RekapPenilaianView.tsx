...
export default function RekapPenilaianView() {
  ...
  const exportCSV = () => {
    const header = ['NIS', 'Nama', 'Kelas', ...displayMapel, 'Rata-rata'];
    const rows = rekap.map(s => [
      s.nis, s.nama, s.kelas,
      ...displayMapel.map(m => s.nilaiPerMapel[m] ?? '-'),
      s.rata ?? '-',
    ]);
    ...
    a.href = url; a.download = 'rekap-penilaian.csv'; a.click();
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-wrap gap-3 items-center">
        ...
        {[
          ...
          <select key={label} className="border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" value={value} onChange={e => setter(e.target.value)}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {rekap.length === 0 && (
                <tr><td colSpan={displayMapel.length + 3} className="px-5 py-12 text-center text-slate-400">Tidak ada data penilaian.</td></tr>
              )}
            </tbody>
          </table>
        </div>
